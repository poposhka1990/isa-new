...
const Home = () => {
  ...
  const calculateVisaDeadline = () => {
    ...
    if (diffMonths >= 19) {
      ...
      const resultRu = `Вам сначала необходимо поменять паспорт, поскольку срока его действия не достаточно для оформления учебной визы. Лучший вариант решения этой проблемы - подача документов на РВПО. Подробнее об этом читайте в разделе Документы -> РВПО. Если вы решили менять паспорт и делаете это в России, то срок подачи документов на студенческую визу: с ${formatDisplayDate(deadlineStart)} по ${formatDisplayDate(deadlineEnd)}. Если вы делаете это в своей стране, вам нужно будет запросить у нас новое приглашение.${calendarLinkText}`;
      const resultEn = `You need to change your passport before, since it is not effective enough for a student visa. The best solution to the problem is applying for Student Residence. Read about it on Documents page -> Student Residence. If you still decided to change passport and do so in Russia, your deadline for applying for a student visa is from ${formatDisplayDate(deadlineStart)} to ${formatDisplayDate(deadlineEnd)}. If you do so in your home country, you will need to ask us for a new invitation.${calendarLinkText}`;
